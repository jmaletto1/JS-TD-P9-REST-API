  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!courseEntry) {
        res.status(404).json({
          msg:
            "Unfortunately you have visited a course that no longer exists! Hot diggity.",
        });
      }
      if (req.currentUserId !== courseEntry.userId) {
        res
          .status(403)
          .json({ msg: "You are not the owner of this route! Be gone!" });
      }
      if (courseEntry) {
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.currentUserId === course.userId) {
      ...
      res.status(400).json({
        msg:
          "I'm afraid you are not the owner of this course! Therefore you cannot delete it.",
      });
    }
  })
);
